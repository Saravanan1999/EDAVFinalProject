...
analyze_event_impact <- function(stock_data, event_date) {
  ...
  stock_columns <- setdiff(names(relevant_prices), "Date") # Exclude 'Date' from calculations
...
# Plot daily returns
ggplot(tech_data_clean, aes(x = date, y = daily_return, color = Source)) +
  geom_line() +
  labs(title = "Daily Returns by Stock", x = "Date", y = "Daily Return (%)") +
  theme_minimal()
```
