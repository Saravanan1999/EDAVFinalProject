...
analyze_event_impact <- function(stock_data, event_date) {
  ...
  pre_event_date <- event_date - days(5)
  post_event_date <- event_date + days(5)
...
# Plot results using facet_grid
ggplot(plot_data, aes(x = Date, y = Return, color = Stock)) +
  geom_line() +
  geom_vline(data = events, aes(xintercept = date), color = 'red', linetype = 'dashed') +
  labs(title = "Stock Price Changes Around Major Events",
       x = "Date",
       y = "Percentage Change (%)") +
  theme_minimal() +
  ...
  facet_grid(rows = vars(Stock), scales = "free_y")  # Facet by stock with free y-axis
