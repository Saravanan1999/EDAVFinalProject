...
```{r}
BAC <- read.csv(BAC_csv_path)
```

```{r}
GS <- read.csv(GS_csv_path)
```

```{r}
JPM <- read.csv(JPM_csv_path)
```

```{r}
MS <- read.csv(MS_csv_path)
```

```{r}
WFC <- read.csv(WFC_csv_path)
```
...
```{r}
aapl = read.csv(AAPL_csv_path)
goog = read.csv(GOOG_csv_path)
meta = read.csv(META_csv_path)
msft = read.csv(MSFT_csv_path)
nvda = read.csv(NVDA_csv_path)
#missing data
datasets <- list(
  "AAPL" = aapl,
  "GOOG" = goog,
